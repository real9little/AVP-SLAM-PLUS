...
void cameraCloudHandler(
  ...
  if (useNDT) {
    ...
    if (!initPose) {
      if (ndt.getFitnessScore() > 0.1) return;
    } else {
      initPose = true;
    }
  ...
  if (useICP) {
    ...
    //   initial registration should be strictly
    if (!initPose) {
      if (icp.getFitnessScore() > 0.1) return;
    } else {
      initPose = true;
    }
  ...
  if (currentPitch > 10) currentPitch = 0;
  transWorldCurrent = pcl::getTransformation(
      currentX, currentY, currentZ, currentRoll, currentPitch, currentYaw);
  ...
  std::cout << "now robot is in x " << currentX << " y " << currentY << "  z  "
            << currentZ << std::endl;
  std::cout << " now robot is in  roll " << currentRoll << "  pitch "
            << currentPitch << "  yaw  " << currentYaw << std::endl;
...
void systemInit() {
  ...
  //   if want to be relocated everywhere, should introduced other method.
  //   machine learing method is a good choice
...
// get initial pose from outer program ;For example ,judging position of robot
// from gps or mannual marker
void initposeHandler(
    const geometry_msgs::PoseWithCovarianceStampedConstPtr &initpose) {
  ...
  double rol, pit, yaw;
  tf::Matrix3x3(tf::Quaternion(tempx, tempy, tempz, tempw))
      .getRPY(rol, pit, yaw);
...
int main(int argc, char *argv[]) {
  ...
  ros::Subscriber subInitpose =
      nh.subscribe<geometry_msgs::PoseWithCovarianceStamped>(
          "/initialposeGive", 1, initposeHandler,

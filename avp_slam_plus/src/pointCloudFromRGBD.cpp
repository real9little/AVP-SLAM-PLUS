...
void removeUnsynData(
    double &timeMax, double &timeCameraPoints,
    std::queue<sensor_msgs::PointCloud2ConstPtr> &cameraCloudBuf) {
...
int main(int argc, char *argv[]) {
  ...
  ros::Subscriber subcamera0Cloud = nh.subscribe<sensor_msgs::PointCloud2>(
      "/camera0/depth/color/points", 100, camera0CloudHandler);
  ros::Subscriber subcamera1Cloud = nh.subscribe<sensor_msgs::PointCloud2>(
      "/camera1/depth/color/points", 100, camera1CloudHandler);
  ros::Subscriber subcamera2Cloud = nh.subscribe<sensor_msgs::PointCloud2>(
      "/camera2/depth/color/points", 100, camera2CloudHandler);
  ros::Subscriber subcamera3Cloud = nh.subscribe<sensor_msgs::PointCloud2>(
      "/camera3/depth/color/points", 100, camera3CloudHandler);
  ros::Subscriber subcamera4Cloud = nh.subscribe<sensor_msgs::PointCloud2>(
      "/camera4/depth/color/points", 100, camera4CloudHandler);
  ros::Subscriber subcamera5Cloud = nh.subscribe<sensor_msgs::PointCloud2>(
      "/camera5/depth/color/points", 100, camera5CloudHandler);
  ...
  while (ros::ok()) {
    ros::spinOnce();
    //  synchronism point cloud of multi cameras according to timestamp
    if (!camera0CloudBuf.empty() && !camera1CloudBuf.empty() &&
        !camera2CloudBuf.empty() && !camera3CloudBuf.empty() &&
        !camera4CloudBuf.empty() && !camera5CloudBuf.empty()) {
      ...
      if ((timeCamera0Points != timeCamera1Points) ||
          (timeCamera0Points != timeCamera2Points) ||
          (timeCamera0Points != timeCamera3Points) ||
          (timeCamera0Points != timeCamera4Points) ||
          (timeCamera0Points != timeCamera5Points)) {
      ...
      *camera5Cloud = camera5CloudDS;
      ...
      for (int i = 0; i < camera1PointSize; i++) {
        transformToFrame(&camera1Cloud->points[i], &camera1Cloud->points[i],
                         q_c0c1, t_c0c1);
      }

      for (int i = 0; i < camera2PointSize; i++) {
        transformToFrame(&camera2Cloud->points[i], &camera2Cloud->points[i],
                         q_c0c2, t_c0c2);
      }

      for (int i = 0; i < camera3PointSize; i++) {
        transformToFrame(&camera3Cloud->points[i], &camera3Cloud->points[i],
                         q_c0c3, t_c0c3);
      }

      for (int i = 0; i < camera4PointSize; i++) {
        transformToFrame(&camera4Cloud->points[i], &camera4Cloud->points[i],
                         q_c0c4, t_c0c4);
      }

      for (int i = 0; i < camera5PointSize; i++) {
        transformToFrame(&camera5Cloud->points[i], &camera5Cloud->points[i],
                         q_c0c5, t_c0c5);
      }
      ...
      *cameraFrameCloud = *cameraFrameCloud + *camera5Cloud;
      ...
      removeClosedPointCloud(*cameraFrameCloud, *cameraFrameCloud,
                             closePointThresh);

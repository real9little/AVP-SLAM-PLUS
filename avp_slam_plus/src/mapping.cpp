...
pcl::PointCloud<PointType>::Ptr currentFeatureCloud(
    new pcl::PointCloud<PointType>());
pcl::PointCloud<PointType>::Ptr currentFeatureCloudInWorld(
    new pcl::PointCloud<PointType>());
...
std::string mapSaveLocation =
    "/home/lgt/multicamera_ws/src/avp_slam/data/icpLaserCloud.pcd";
...
void cameraCloudHandler(
  ...
  //   for real scene,should adapt deep learning method,detecting valid feature
  //   point
  currentFeatureCloud->clear();

  /// @comment select the features using rgb information value only \
  /// such a naive strategy
  for (size_t i = 0; i < cameraCloudIn.points.size(); i++) {
    PointType pi = cameraCloudIn.points[i];
    int r = int(pi.r);
    int g = int(pi.g);
    int b = int(pi.b);
    if (r < invalidColorThresh && b < invalidColorThresh &&
        g < invalidColorThresh) {
      continue;
    }
    currentFeatureCloud->push_back(pi);
  }

  /// @comment  broadcast current feature point cloud
  ...
  if (currentFeatureCloud->points.size() < 10) return;
  ...
  /// @comment compute robot's pose with method of ndt
  if (useNDT) {
    ...
    pcl::PointCloud<PointType>::Ptr transCurrentCloudInWorld(
        new pcl::PointCloud<PointType>());
    ndt.align(*transCurrentCloudInWorld, transWorldCurrent.matrix());
    if (ndt.hasConverged() == false ||
        ndt.getFitnessScore() > ndtFitnessScoreThresh) {
      std::cout << "ndt locolization failed    the score is   "
  ...
  if (useICP) {
    ...
    if (icp.hasConverged() == false ||
        icp.getFitnessScore() > icpFitnessScoreThresh) {
      std::cout << "ICP locolization failed    the score is   "
  ...
  pcl::getTranslationAndEulerAngles(transWorldCurrent, currentX, currentY,
                                    currentZ, currentRoll, currentPitch,
                                    currentYaw);

  tf::TransformBroadcaster tfMap2Camera;
  tf::Transform mapToCamera = tf::Transform(
      tf::createQuaternionFromRPY(currentRoll, currentPitch, currentYaw),
      tf::Vector3(currentX, currentY, currentZ));
  tfMap2Camera.sendTransform(tf::StampedTransform(mapToCamera, ros::Time::now(),
                                                  "/map", "/camera0_link"));

  // broacast pose of robot
  geometry_msgs::Quaternion cameraPoseQuat =
      tf::createQuaternionMsgFromRollPitchYaw(currentRoll, currentPitch,
                                              currentYaw);
  ...
  std::cout << "now robot is in x " << transWorldCurrent(0, 3) << " y "
            << transWorldCurrent(1, 3) << "  z  " << transWorldCurrent(2, 3)
            << std::endl;

  /// @comment the author of this code is stupid again \
  /// pcl provides an implementation of point cloud transformation \
  /// please check pcl::common::transform for detailed info
  *currentFeatureCloudInWorld =
      *transformPointCloud(currentFeatureCloud, transWorldCurrent);
  ...
  //  create new keyFrame, and add pointCloud to global map
  double dis = (currentX - lastPoseX) * (currentX - lastPoseX) +
               (currentY - lastPoseY) * (currentY - lastPoseY) +
               (currentZ - lastPoseZ) * (currentZ - lastPoseZ);
  frameCount++;

  /// @comment accumulate global point cloud
...
int main(int argc, char *argv[]) {
  ...
  /// @comment the last value below </home/lgt/multicamera_ws/src/avp_slam/data/>
  /// is the default location, will be replace by the ones listed in yaml file
  nh.param<std::string>("mapSaveLocation", mapSaveLocation,
                        "/home/lgt/multicamera_ws/src/avp_slam/data/");

  /// @comment if the distance estimated from odometer, concatenate the point
  /// cloud, otherwise the accumulated point cloud will be too large
  nh.param<double>("odomKeyFramDisThresh", odomKeyFramDisThresh, 1.0);

  /// @comment extract semantic information using color in a naive way
  nh.param<int>("invalidColorThresh", invalidColorThresh, 60);

  /// @comment run icp for point cloud registration
  nh.param<bool>("useICP", useICP, true);

  /// @comment icp parameters, please visit pcl documentation for detailed info
  nh.param<double>("icpMaxCorrespondenceDistance", icpMaxCorrespondenceDistance,
                   20);
  ...
  nh.param<double>("icpEuclideanFitnessEpsilon", icpEuclideanFitnessEpsilon,
                   0.001);
  nh.param<double>("icpFitnessScoreThresh", icpFitnessScoreThresh, 0.3);
  nh.param<float>("pointCloudLeafSize", pointCloudLeafSize, 0.1);

  /// @comment estimate robot pose using NDT (Normal Distribution Transform)
  nh.param<bool>("useNDT", useNDT, true);

  /// @comment NDT configuration parameters
  ...
  /// @comment point cloud published by the previous node
  ros::Subscriber subcameraCloud = nh.subscribe<sensor_msgs::PointCloud2>(
      "/cameraCloudFrame", 100, cameraCloudHandler);

  /// @comment subscribe from the controller
  ros::Subscriber subSaveMap = nh.subscribe<geometry_msgs::PoseStamped>(
      "/move_base_simple/goal", 10, saveMap_callback);

  pubCurrentFeature =
      nh.advertise<sensor_msgs::PointCloud2>("/currentFeature", 100);
  pubCurrentFeatureInWorld =
      nh.advertise<sensor_msgs::PointCloud2>("/currentFeatureInWorld", 100);
  pubGlobalFeature =
      nh.advertise<sensor_msgs::PointCloud2>("/globalFeatureMap", 100);

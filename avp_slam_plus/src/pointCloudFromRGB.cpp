...
pcl::PointCloud<PointType>::Ptr cameraFrameCloud(
...
void camera0CloudHandler(
    const sensor_msgs::PointCloud2ConstPtr& cameraCloudMsg) {
  mBuf.lock();
  camera0CloudBuf.push(cameraCloudMsg);
  mBuf.unlock();
}

void camera1CloudHandler(
    const sensor_msgs::PointCloud2ConstPtr& cameraCloudMsg) {
  mBuf.lock();
  camera1CloudBuf.push(cameraCloudMsg);
  mBuf.unlock();
}
void camera2CloudHandler(
    const sensor_msgs::PointCloud2ConstPtr& cameraCloudMsg) {
  mBuf.lock();
  camera2CloudBuf.push(cameraCloudMsg);
  mBuf.unlock();
}
void camera3CloudHandler(
    const sensor_msgs::PointCloud2ConstPtr& cameraCloudMsg) {
  mBuf.lock();
  camera3CloudBuf.push(cameraCloudMsg);
  mBuf.unlock();
}
void camera4CloudHandler(
    const sensor_msgs::PointCloud2ConstPtr& cameraCloudMsg) {
  mBuf.lock();
  camera4CloudBuf.push(cameraCloudMsg);
  mBuf.unlock();
}
void camera5CloudHandler(
    const sensor_msgs::PointCloud2ConstPtr& cameraCloudMsg) {
  mBuf.lock();
...
void degToQuan(double& rotateDeg, Eigen::Quaterniond& q) {
  geometry_msgs::Quaternion camera0xQuat =
      tf::createQuaternionMsgFromRollPitchYaw(0, rotateDeg, 0);
  q.w() = camera0xQuat.w;
  q.x() = camera0xQuat.x;
  q.y() = camera0xQuat.y;
  q.z() = camera0xQuat.z;
...
void systemInit() {
  /// @comment init 4x4 transformation matrix from camera to body
  R1R = Eigen::AngleAxisd(-rotateDeg1, Eigen::Vector3d(0, 1, 0))
            .toRotationMatrix();
  T1R << 0, 0, 0;

  R2R = Eigen::AngleAxisd(-rotateDeg2, Eigen::Vector3d(0, 1, 0))
            .toRotationMatrix();
  T2R << 0, 0, 0;

  R3R = Eigen::AngleAxisd(-rotateDeg3, Eigen::Vector3d(0, 1, 0))
            .toRotationMatrix();
  T3R << 0, 0, 0;

  R4R = Eigen::AngleAxisd(-rotateDeg4, Eigen::Vector3d(0, 1, 0))
            .toRotationMatrix();
  T4R << 0, 0, 0;

  R5R = Eigen::AngleAxisd(-rotateDeg5, Eigen::Vector3d(0, 1, 0))
            .toRotationMatrix();
  T5R << 0, 0, 0;

  K0 = K;
  RT0 << R0(0, 0), R0(0, 1), T0(0), R0(1, 0), R0(1, 1), T0(1), R0(2, 0),
      R0(2, 1), T0(2);

  K1 = K;
  R1 = R1R * R0;
  T1 = R1R * T0 + T1R;
  RT1 << R1(0, 0), R1(0, 1), T1(0), R1(1, 0), R1(1, 1), T1(1), R1(2, 0),
      R1(2, 1), T1(2);

  K2 = K;
  R2 = R2R * R0;
  T2 = R2R * T0 + T2R;
  RT2 << R2(0, 0), R2(0, 1), T2(0), R2(1, 0), R2(1, 1), T2(1), R2(2, 0),
      R2(2, 1), T2(2);

  K3 = K;
  R3 = R3R * R0;
  T3 = R3R * T0 + T3R;
  RT3 << R3(0, 0), R3(0, 1), T3(0), R3(1, 0), R3(1, 1), T3(1), R3(2, 0),
      R3(2, 1), T3(2);

  K4 = K;
  R4 = R4R * R0;
  T4 = R4R * T0 + T4R;
  RT4 << R4(0, 0), R4(0, 1), T4(0), R4(1, 0), R4(1, 1), T4(1), R4(2, 0),
      R4(2, 1), T4(2);

  K5 = K;
  R5 = R5R * R0;
  T5 = R5R * T0 + T5R;
  RT5 << R5(0, 0), R5(0, 1), T5(0), R5(1, 0), R5(1, 1), T5(1), R5(2, 0),
      R5(2, 1), T5(2);
}

void transformToFrame(PointType const* const pi, PointType* const po,
...
void removeUnsynData(double& timeMax, double& timeCameraImage,
...
void calCloudFromImage(Eigen::Matrix3d& K, Eigen::Matrix3d& RT,
  ...
  for (int i = 0; i < row; i = i + imageRowIncrease) {
    ...
    for (int j = 0; j < col; j = j + imageColIncrease) {
      ...
      /// @comment compute uv -> 3d
      u1 = KInv * u;

      /// @comment from 3d to body coordinate frame
      u1 = RTInv * u1;
      u1(0) = u1.x() / u1.z();
      u1(1) = u1.y() / u1.z();
      double dis = sqrt(u1.x() * u1.x() + u1.y() * u1.y());

      if (dis > cameraRealiableDis) continue;


      /// @comment point cloud generation, z value is zero (ground)
      PointType po;
      po.x = u1.x();
      po.y = u1.y();
      po.z = 0;
...
int main(int argc, char* argv[]) {
  ...
  /// @comment ~ extrinsic parameter, especially rotation
  ...
  /// @comment point cloud voxel grid filter leaf size
  nh.param<float>("pointCloudLeafSize", pointCloudLeafSize, 0.1);

  /// @comment image sub-sampling step size (row/col)
  nh.param<int>("imageRowIncrease", imageRowIncrease, 1);
  nh.param<int>("imageColIncrease", imageColIncrease, 2);

  /// @comment disparity threshold
  nh.param<double>("cameraRealiableDis", cameraRealiableDis, 8.0);

  /// @comment image semantic segmentation using rgb value, needs to be replaced
  /// by DL algorithm in real scenarios
  nh.param<int>("skyColor", skyColor, 178);

  /// @comment intrinsic and extrinsi calibration parameters of cameras
  ...
  K = Eigen::Map<const Eigen::Matrix<double, -1, -1, Eigen::RowMajor>>(
      KDouble.data(), 3, 3);
  R0 = Eigen::Map<const Eigen::Matrix<double, -1, -1, Eigen::RowMajor>>(
      R0Double.data(), 3, 3);
  T0 = Eigen::Map<const Eigen::Matrix<double, -1, -1, Eigen::RowMajor>>(
      T0Double.data(), 3, 1);

  /// @comment subscribed image topics
  ros::Subscriber subCamera0Image =
      nh.subscribe("/camera0/color/image_raw", 100, camera0ImageHandler);
  ros::Subscriber subCamera1Image =
      nh.subscribe("/camera1/color/image_raw", 100, camera1ImageHandler);
  ros::Subscriber subCamera2Image =
      nh.subscribe("/camera2/color/image_raw", 100, camera2ImageHandler);
  ros::Subscriber subCamera3Image =
      nh.subscribe("/camera3/color/image_raw", 100, camera3ImageHandler);
  ros::Subscriber subCamera4Image =
      nh.subscribe("/camera4/color/image_raw", 100, camera4ImageHandler);
  ros::Subscriber subCamera5Image =
      nh.subscribe("/camera5/color/image_raw", 100, camera5ImageHandler);

  /// @comment published point cloud topics
  pubCameraCloudFrame =
      nh.advertise<sensor_msgs::PointCloud2>("/cameraCloudFrame", 100);
  ...
  while (ros::ok()) {
    ros::spinOnce();
    ///  synchronism images of multi cameras according to timestamp
    /// @comment this synchronization is actually not perfect
    /// please refer to (http://wiki.ros.org/message_filters)\
    /// for the detailed steps to write sync code between multiple topics
    if (!camera0ImageBuf.empty() && !camera1ImageBuf.empty() &&
        !camera2ImageBuf.empty() && !camera3ImageBuf.empty() &&
        !camera4ImageBuf.empty() && !camera5ImageBuf.empty()) {
      ...
      if ((timeCamera0Image != timeCamera1Image) ||
          (timeCamera0Image != timeCamera2Image) ||
          (timeCamera0Image != timeCamera3Image) ||
          (timeCamera0Image != timeCamera4Image) ||
          (timeCamera0Image != timeCamera5Image)) {
      ...
      /// get cv_ptr from the message
      /// mutex lock is involved, read related mutex/lock c++ blogs for more info
      mBuf.lock();
      ...
      /// @comment generate point cloud in body frame
      camera0Cloud->clear();
      ...
      downSizeFilter.setLeafSize(pointCloudLeafSize, pointCloudLeafSize,
                                 pointCloudLeafSize);
